...
export class TimeSpan {
  ...
  divide(divisor: number | TimeSpan) {
    if (divisor instanceof TimeSpan) {
      return new TimeSpan(Math.floor(this._ticks / divisor._ticks));
    }
  ...
  toString() {
    ...
    const milliseconds = this.milliseconds >= 100 ? this.milliseconds : this.milliseconds >= 10 ? "0" + this.milliseconds : "00" + this.milliseconds;
  ...
  static fromDates(a: Date, b: Date): TimeSpan {
    ...
    const milliSeconds: number = Math.abs(Math.floor(utc1 - utc2));
