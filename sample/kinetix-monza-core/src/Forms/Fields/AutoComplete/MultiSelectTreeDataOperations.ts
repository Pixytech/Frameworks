...
const mapMultiSelectTreeData = (data: any, options: any) => {
  const { keyGetter, subItemGetter, subItemSetter, checkSetter, expandedSetter, checkIndeterminateSetter, valueMap, expandedMap }: any = options;
...
export const processMultiSelectTreeData = (tree: any, options: any) => {
  const { subItemsField = "items", checkField = "checkField", checkIndeterminateField = "checkIndeterminateField", expandField = "expanded", dataItemKey, value, filter, expanded }: any = options;

  const keyGetter = getter(dataItemKey);
  const filtering = Boolean(filter?.value);
